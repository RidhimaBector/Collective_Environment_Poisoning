...
class VictimAgent_DQN_Pop():
    ...
    def __init__(self, env, MEMORY_SIZE, BATCH_SIZE, GAMMA, EPS_START, EPS_END, EPS_DECAY, EPSILON, EPSILON_DECAY, TARGET_UPDATE, n_states, n_actions, learning_rate=0.001, no_of_agents=1):
        ...
        self.approxQ_data_array, self.LA_array, self.approxQ_array, self.np_array = [], [], [], []
        for k in range(self.no_of_agents):
            self.approxQ_data_array.append([[4]*10 for _ in range(16)]) #[[-1]*10]*16)
            self.LA_array.append(np.ones((16,2))*4)
            self.LA_array[-1][:,0] = np.arange(16)
            self.approxQ_array.append(np.zeros((16,4)))
            self.np_array.append(np)
            self.np_array[k].random.seed(k+1)
    ...
    def reset(self):
        ...
        #self.steps_done = 0        
        self.approxQ_data_array, self.LA_array, self.approxQ_array = [], [], []
        for k in range(self.no_of_agents):
            self.approxQ_data_array.append([[4]*10 for _ in range(16)]) #[[-1]*10]*16) 
            self.LA_array.append(np.ones((16,2))*4)
            self.LA_array[-1][:,0] = np.arange(16)
    ...
    def select_action(self, state, k):
        #global steps_done
        sample = self.np_array[k].random.rand() #random.random()
        ...
        if sample > eps_threshold:
            ...
            return torch.tensor([[self.np_array[k].random.randint(self.n_actions)]], device=device, dtype=torch.long)
    ...
    def train_model(self, num_episodes, t_max):
        ...
        for i_episode in range(num_episodes):
            ...
            for t in range(t_max):
                ...
                action_array = []
                for k in range(self.no_of_agents):
                    action = self.select_action(state_complete_array[k], k).to(device)
...
#               self.env.render()
        ...
        return accuracy_episode_array, accuracy_softmax_episode_array, accuracy_softmax_complete_episode_array, self.LA_array
...
if __name__ == "__main__":
    ...
    while(j > 0):
        ...
        episodes = 250 #200
        t_max = 20
        victim_transitions_array = victim_pop.train_model(episodes, t_max)
