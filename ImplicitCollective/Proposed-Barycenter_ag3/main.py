...
if __name__ == "__main__":
    ...
    for i_episode in range(args.max_episodes_num):
        ...
        for t in range(args.max_timesteps):
            ...
            for k in range(no_of_agents):           
                distance[k+1] = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=0) #system.victim.Q
                distance_grid[k+1] = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=1)
                distance_behavior[k+1] = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=2)
                done_T, accuracy_T, accuracy_sftmx_T, accuracy_sftmx_complete_T = utils_attack.Attack_Done_Identify(env, TARGET, victim_array[k].Q) #system.victim.Q)
                done[k+1] = done_T
