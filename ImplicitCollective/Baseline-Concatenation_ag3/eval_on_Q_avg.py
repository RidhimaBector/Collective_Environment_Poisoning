...
#for k in range(no_of_agents):
    #ae_array[k].load("storage/R_0818/" + str(k) + "_" + str(policy_no_AE) + "_AutoEncoder")
...
def eval_policy(policy):
    ...
    accuracy_victim_timestep = np.zeros((no_of_agents, Tmax*80)) #[] #complete data
    accuracy_atk_timestep = np.zeros((no_of_agents, Tmax)) #[] #complete data
    accuracy_sftmx_victim_timestep = np.zeros((no_of_agents, Tmax*80)) #[] #complete data
    accuracy_sftmx_atk_timestep = np.zeros((no_of_agents, Tmax)) #[] #complete data
    accuracy_sftmx_complete_victim_timestep = np.zeros((no_of_agents, Tmax*80)) #[] #complete data
    ...
    for t in range(Tmax):
        ...
        next_victim_info = ae.Embedding(victim_array[0].MEM)
        for k in range(1,no_of_agents):
            next_victim_info = np.concatenate((next_victim_info, ae.Embedding(victim_array[k].MEM)), axis=1) 
        ...
        for k in range(no_of_agents):
            distance_K.append(utils_attack.Attack_Cost_Compute_K(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=0)) #system.victim.Q
            distance_grid_K.append(utils_attack.Attack_Cost_Compute_K(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=1))
            distance_behavior_K.append(utils_attack.Attack_Cost_Compute_K(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=2))
            distance_W.append(utils_attack.Attack_Cost_Compute_W(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=0)) #system.victim.Q
            distance_grid_W.append(utils_attack.Attack_Cost_Compute_W(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=1))
            distance_behavior_W.append(utils_attack.Attack_Cost_Compute_W(env, INIT_T, victim_array[k].Q, TARGET, cost_matrix, distance_type=2))
        ...
        for k in range(no_of_agents):
            ...
            accuracy_victim_timestep[k, t*80 : (t+1)*80] = np.array(accuracy_array[k])
            accuracy_atk_timestep[k, t] = accuracy_array[k][-1]
            accuracy_sftmx_victim_timestep[k, t*80 : (t+1)*80] = np.array(accuracy_sftmx_array[k])
            accuracy_sftmx_atk_timestep[k, t] = accuracy_sftmx_array[k][-1]
            accuracy_sftmx_complete_victim_timestep[k, t*80 : (t+1)*80] = np.array(accuracy_sftmx_complete_array[k])
...
vic_time =  np.tile((time_compression * avg_step) / 80, no_of_agents) #list(np.array(time_compression) * avg_step)
