...
if __name__ == "__main__":
    ...
    system = System(victim_args, ae_args, no_of_agents)
    ...
    for i_episode in range(args.max_episodes_num):
        ...
        for k in range(no_of_agents):
            system.victim_array[k].reset() #system_array[k].victim.reset()
        ...
        for t in range(args.max_timesteps):
            ...
            next_victim_info = system.ae.Embedding(system.victim_array[0].MEM)
            for k in range(1,no_of_agents):
                next_victim_info = np.concatenate((next_victim_info, system.ae.Embedding(system.victim_array[k].MEM)), axis=1) 
            ...
            for k in range(no_of_agents):           
                distance[k+1] = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, system.victim_array[k].Q, TARGET, cost_matrix, distance_type=0) #system.victim.Q
                distance_grid[k+1] = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, system.victim_array[k].Q, TARGET, cost_matrix, distance_type=1)
                distance_behavior[k+1] = - utils_attack.Attack_Cost_Compute_K(env, INIT_T, system.victim_array[k].Q, TARGET, cost_matrix, distance_type=2)
                done_T, accuracy_T, accuracy_sftmx_T, accuracy_sftmx_complete_T = utils_attack.Attack_Done_Identify(env, TARGET, system.victim_array[k].Q) #system.victim.Q)
                done[k+1] = done_T
        ...
        if no_episodes % args.eval_freq_episode == 0:
            ...
            #for k in range(no_of_agents):
                #system_array[k].ae.save(f"./{model_dir}/{k}_{no_episodes}")
